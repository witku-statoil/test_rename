if object_id('stg_jde.tmp_jde_statistics_previous','U') is not null
    drop table stg_jde.tmp_jde_statistics_previous;
if object_id('stg_jde.tmp_jde_statistics','U') is not null
    rename object stg_jde.tmp_jde_statistics to tmp_jde_statistics_previous;

create table stg_jde.tmp_jde_statistics
with
(
    HEAP
...
(
    ...
    from (
        ...
        from (select table_cd,table_cdc_type,table_cdc_status_cd,table_cdc_status_msg/*,table_cdc_dt*/,table_cdc_scn_from,table_cdc_scn_to,table_cdc_exp_file_name,table_ignore_chk_ind
					...
					,table_cdc_chk_col_1,table_cdc_chk_col_2,table_cdc_chk_col_3,table_cdc_chk_col_4,table_cdc_chk_col_5
                    ,'[stg_jde].[tmp_' + lower(table_cd) + (case when table_cdc_type = 'FULL' then '_init' else '_cdc' end) + ']' as full_stg_table_cd
                    ,'[rep_jde].['     + lower(table_cd)                                                                    + '_new]' as full_rep_table_cd
                    ,row_number() over (partition by table_cd
                                         ...
                                         as rown
              from [stg_jde].[ext_jde_statistics]
